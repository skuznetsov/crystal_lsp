...
COMPILER="$1"
TMP_DIR="$(mktemp -d "$ROOT_DIR/tmp/nilable-array-idxq.XXXXXX")"
...
if [[ $compile_rc -eq 0 && $run_rc -eq 0 && "$stdout" == "true" ]]; then
  echo "not reproduced"
  exit 1
fi

echo "reproduced: nilable Array#[]? misresolved away from the Int32 query overload"
exit 0
