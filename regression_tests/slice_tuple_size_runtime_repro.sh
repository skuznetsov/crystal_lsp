...
COMPILER="$1"
TMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/slice_tuple_size.XXXXXX")"
...
if [[ $compile_status -ne 0 ]]; then
  echo "reproduced: slice tuple size sample failed to compile"
  echo "--- compile stdout ---"
  cat "$COMPILE_STDOUT"
  echo "--- compile stderr ---"
  cat "$COMPILE_STDERR"
...
echo "reproduced: slice tuple size sample has wrong runtime signature"
exit 0
