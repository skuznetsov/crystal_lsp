...
COMPILER="$1"
TMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/pointer_tuple_slot.XXXXXX")"
...
cat >"$SRC" <<'CR'
ptr = Pointer(typeof({1_u32, 3_u32})).malloc(1)
ptr[0] = {1_u32, 3_u32}
puts ptr[0][0]
puts ptr[0][1]

nested = Pointer(typeof({ {1_u32, 3_u32}, 3_u32 })).malloc(1)
nested[0] = { {1_u32, 3_u32}, 3_u32 }
puts nested[0][0][0]
puts nested[0][0][1]
puts nested[0][1]
CR
...
if [[ $compile_status -ne 0 ]]; then
  echo "reproduced: pointer tuple slot sample failed to compile"
  echo "--- compile stdout ---"
  cat "$COMPILE_STDOUT"
  echo "--- compile stderr ---"
  cat "$COMPILE_STDERR"
...
if [[ $run_status -eq 139 ]] || grep -Eq 'Segmentation fault|Segfault|signal 11|status: 139|exit 139|EXC_BAD_ACCESS' "$RUN_LOG"; then
  echo "reproduced: pointer tuple slot sample crashes at runtime"
  exit 0
fi

if [[ $run_status -eq 0 ]]; then
  echo "reproduced: pointer tuple slot sample returned unexpected output"
  exit 0
fi

echo "reproduced: unexpected pointer tuple slot signature"
exit 0
