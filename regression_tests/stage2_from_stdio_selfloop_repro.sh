...
OUT_DIR="${TMPDIR:-/tmp}/stage2_from_stdio_selfloop_repro"
...
from_stdio_hits="$(rg -c 'frame #0: .*IO\$CCFileDescriptor\$Dfrom_stdio\$\$Int32' "$LLDB_LOG" || true)"
self_loop_hits="$(
  perl -ne '
...
echo "run_log: $RUN_LOG"
echo "lldb_log: $LLDB_LOG"
echo "from_stdio_frame_hits: ${from_stdio_hits:-0}"
echo "self_branch_hits: ${self_loop_hits:-0}"

if [ "${from_stdio_hits:-0}" -ge 1 ] && [ "${self_loop_hits:-0}" -ge 1 ]; then
  echo "reproduced (IO::FileDescriptor.from_stdio self-loop in stage2 compiler)"
  exit 0
fi
