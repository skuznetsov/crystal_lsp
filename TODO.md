...
- **Current smallest standalone parser-shape oracle**:
  ...
  - the current committed witness now reduces further to pure generic syntax: `typed generic param(x : A(B)) -> @y = 1 -> z = @y -> bare z read -> pure nested if/if/tail-if`, so the active carrier no longer requires any stdlib type names, param-to-ivar assignment, shorthand `@args` params, `@args.size`, `@args[i]`, string compare, or local assignment from the indexed read
  ...
  - local refutation ledger on this witness:
    ...
    - `tmp_parse_args_shape_init_unknown_nongeneric_literal_ivar_alias_read_if_true_tailand.cr` stays green `5/5`, so nongeneric typed constants are not enough without the generic annotation path
    - `tmp_parse_args_shape_init_unknown_generic_literal_direct_ivar_read_if_true_tailand.cr` stays green `5/5`, so the active red shape still needs local alias assignment+read, not just a direct ivar read
    - `tmp_parse_args_shape_init_int_param_literal_ivar_alias_read_if_true_tailand.cr` stays green `5/5`, so plain typedness is not enough
    - `tmp_parse_args_shape_init_array_int_literal_ivar_alias_read_if_true_tailand.cr` is red on attempt `1`, and `tmp_parse_args_shape_init_typed_param_literal_ivar_value_alias_read_if_true_tailand.cr` is also red on attempt `1`, so neither inner generic arg names nor alias identifier names are part of the remaining live carrier
