...
- **Fix applied**: normalize tuple literal type names during namespace resolution (avoid `Unicode::{Int32, ...}`); tuple literals now resolve to `Tuple(...)`, removing `Unicode::{Int32, Int32, Int32}#[]$Int32` unresolved calls in `/tmp/fib_unresolved.log` (2026-02-xx).
