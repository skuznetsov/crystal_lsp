...
- **2026-02-24 (latest): stage1 `Object#to_s` crash fixed; stage2 release links again but runtime stability still broken**
  ...
  - Fresh benchmark with current tree (before `Array#join` cast patch):
  ...
  - Stage2 status on same run:
    ...
    - failed in `opt` (no output binary), blocker was:
  ...
  - Fresh benchmark after follow-up fix:
    - stage1 (original compiler): `crystal build --release src/crystal_v2.cr -o /tmp/stage1_rel_20260224_fix8`
      - **real 429.70s**
    - stage2 (self-hosted, timeout-guarded): `/tmp/stage1_rel_20260224_fix8 --release src/crystal_v2.cr -o /tmp/stage2_rel_20260224_fix8`
      - **real 147.68s**, exit 0 (binary produced).
    - speedup stage2 vs stage1: **~2.91x** (delta ~-282.02s).
  - Current blocker:
    - stage2 binary is still unstable at runtime:
      - `/tmp/stage2_rel_20260224_fix8 --version` -> `Unhandled exception:`
      - `regression_tests/run_all.sh /tmp/stage2_rel_20260224_fix8` -> **0/56 passed** (all compile fail with `Unhandled exception:`).
    - reference baseline:
      - `regression_tests/run_all.sh /tmp/stage1_rel_20260224_fix8` -> **49/56 passed**.
...
- **HIR union-cover selection hotspot reduced (2026-02-18)** —
  ...
  Root hotspot before fix (`/tmp/stage1_profile_nocache.sample.txt`):
  - `String#index` (1652 top-stack hits), `GC_mark_from` (2164),
    `AstToHir#find_covering_union_type` (360), with stack dominated by
    `lower_if -> find_covering_union_type -> GC`.
  Changes in `src/compiler/hir/ast_to_hir.cr`:
  - replaced `required_types.map(...).uniq` with manual de-dup over small
    `Array(MIR::TypeRef)` (no intermediate array chain);
  - switched scan source from full `@module.types` to existing
    `@union_descriptors` only;
  - removed per-candidate `descriptor.variants.map(&.type_ref)` allocations;
  - added branch pruning (`variant_count < required.size`,
    `variant_count >= best_variant_count`) and exact-cover early break.
  Validation:
  - release build: `crystal build src/crystal_v2.cr --release -o /tmp/crystal_v2_rel_unioncover_opt`
    => `real 411.83s`;
  - stage1 (release, no pipeline cache):
    `CRYSTAL_V2_PIPELINE_CACHE=0 /tmp/crystal_v2_rel_unioncover_opt build src/crystal_v2.cr --release --no-ast-cache --no-llvm-cache -o /tmp/crystal_v2_stage1_rel_unioncover_opt`
    => `real 115.23s` (previous comparable run: `119.71s`);
  - regressions: `regression_tests/run_all.sh /tmp/crystal_v2_rel_unioncover_opt`
    => `42 passed, 0 failed`;
  - bootstrap smoke:
    `/tmp/crystal_v2_rel_unioncover_opt examples/bootstrap_array.cr -o /tmp/bootstrap_array_unioncover_opt && scripts/run_safe.sh /tmp/bootstrap_array_unioncover_opt 10 768`
    => `EXIT 0`.
...
- [x] **Audit int/ptr → float conversions**: ensure `uitofp`/`sitofp` are correct for unsigned/signed and pointer casts across all backends; add specs for representative signed/unsigned/ptr cases.
  - **Update (2026-01-30)**: audited HIR numeric conversions + MIR CastKind mapping and LLVM backend float casts.
    ...
    - **Update (2026-02-18)**: fixed `emit_extern_call` fixed-arg coercion path to use `fptoui` (not always `fptosi`) for `float -> UInt*` when target parameter type is known from `matching_func`. Added regression spec (`float64 arg -> uint32 callee` via `ExternCall`) in `spec/mir/llvm_backend_spec.cr`; validation: `crystal spec spec/mir/llvm_backend_spec.cr` => `60 examples, 0 failures`; `crystal build src/crystal_v2.cr --release -o /tmp/crystal_v2_rel_fptoui`; `regression_tests/run_all.sh /tmp/crystal_v2_rel_fptoui` => `42 passed, 0 failed`.
...
- [x] **ARM/AArch64 alignment audit**: verify stack/alloca/struct field alignment for ARM targets (incl. AArch64); ensure align=4 where required and matches LLVM target ABI.
  ...
  - **Update (2026-02-18)**: `mir` TypeRegistry and allocation defaults now use target pointer-width constants instead of hardcoded `8` (`STRING`/`POINTER` primitive layout + default alloc alignment). Validation: `crystal spec spec/mir/mir_spec.cr spec/mir/abi_layout_spec.cr` => `26 examples, 0 failures`; `crystal build src/crystal_v2.cr --release -o /tmp/crystal_v2_rel_mirptr`; `regression_tests/run_all.sh /tmp/crystal_v2_rel_mirptr` => `42 passed, 0 failed`.
