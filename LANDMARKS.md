...
- [LM-p1-ir-shape-guard-2026-04-19]: `regression_tests/p1_ir_shape_check.sh` is the current focused shape guard for the hybrid closure-env ABI boundary. It emits HIR and LLVM for a fixture that combines `call_block(&block)` with a zero-capture proc literal, dynamically extracts the captured `__crystal_block_proc_N` symbol from HIR, and verifies: boxed `make_closure` metadata is present, `make_proc` materializes Proc values, the focused heap `Proc#call` has no hidden capture args, LLVM does not use `%__crystal_proc` as a by-value Proc type, the captured block receives `ptr %__closure_env`, and heap Proc object storage/dispatch uses fn@0/env@8. Verification: `bash -n regression_tests/p1_ir_shape_check.sh` is clean; `regression_tests/p1_ir_shape_check.sh bin/crystal_v2` prints `p1_ir_shape_ok captured_fn=__crystal_block_proc_1` and exits 0; adjacent spawn/conditional/escaping closure forward guards still report fixed-state exit 1. Boundary: this is an additive guard for the present hybrid ABI shape, not proof that legacy `@proc_captures_by_value` / `@closure_ref_cells` cleanup is complete. {F:0.90/G:0.65/R:0.92} [verified]
- [LM-p1-hybrid-boundary-guard-2026-04-19]: `regression_tests/p1_hybrid_boundary_guard.sh` is the current source-level guard for intentional pre-cleanup closure-env ABI anchors. It verifies the presence of `@proc_captures_by_value`, hidden capture append paths, `@closure_ref_cells` / `@closure_ref_prefer_cell` fallback anchors, and the dual-mode `lower_block_to_proc` split where heap paths return `emit_make_proc_value(...)` while legacy raw callback paths still use closure cells plus `FuncPointer`. It also checks that the plan/TODO/collab docs still mark this as a hybrid boundary. Verification: `bash -n regression_tests/p1_hybrid_boundary_guard.sh` is clean; `regression_tests/p1_hybrid_boundary_guard.sh` prints `p1_hybrid_boundary_ok` and exits 0; `regression_tests/p1_ir_shape_check.sh bin/crystal_v2` remains green. Boundary: this is presence-based and should be updated or removed together with the coupled ABI cleanup that intentionally deletes those anchors. {F:0.78/G:0.58/R:0.90} [verified]
