...
containment branch was real: successive generated-stage4 rebuilds from
`/tmp/crystal_v2_hir_stage3probe` with local hot-path rewrites in
`src/compiler/frontend/lexer.cr`, `src/compiler/hir/ast_to_hir.cr`, and
`src/compiler/cli.cr` moved the exact `hello`/string-only no-prelude carriers
forward from the old lexer runaway and early `CLI#compile`
`Enumerable(...NotFoundError)#upto` abort into a later MIR frontier.
Specifically, the rebuilt `/tmp/crystal_v2_stage4_from_stage3_hotloops5`
converges under LLDB on
`Crystal::MIR::HIRToMIRLowering#register_enum_types -> Set(String)#includes?`,
while one observed full `stage3 -> stage4` rebuild improved from the earlier
`total=254731.8ms` baseline to `total=241190.6ms`. Second, adversary exact
reducers falsified the stronger root-cause claim: `/tmp/lexer_symbol.cr`,
`/tmp/lexer_colons.cr`, and `/tmp/lexer_label.cr` still drive
`Parser#token_preload_capacity` into 10M+ tokens on the latest generated
artifact, even with `CRYSTAL_V2_TRUST_SLICE_ADDR=1`. That means the current
branch is a genuine containment checkpoint for the old string/comment + early
CLI fronts, but not a compiler-wide cure for omitted-default-arg/cursor helper
miscompilation. Boundary: treat `lex_string`/`emit_self`/`CLI def_nodes`
rewrites as verified local hardening and perf movement, but keep the broader
root-cause hypothesis open until the `:foo` / `::` / `foo:` reducers also stop
running away. {F/G/R: 0.96/0.63/0.98} [verified]
...
`MakeClosure`/parent-closure-scope path for plain proc literals, while the live
HIR output showed `func @foo` returning `%0 = func_pointer @__crystal_proc_0`
and no `ScopeKind::Closure` in the parent function. Source inspection confirms
that `lower_proc_literal` intentionally emits standalone top-level proc
...
`Cyclic` or `FFIExposed` should immediately choose `GC`. Third, the taint side
stayed green under direct regression coverage:
`../crystal/bin/crystal spec spec/hir/taint_analysis_spec.cr --error-trace`,
which proves the `to_unsafe` carrier still reaches strategy assignment marked
as `FFIExposed`. The verified fix is narrow and local:
...
[LM-236|verified]: the current self-hosted CLI hardening checkpoint is a two-part boundary fix, not a single-line `Array(ExprId)` duplication tweak. Fresh `host -> stage1 -> stage2` with `/tmp/stage1_cli_exprdup_final` and `/tmp/stage2_cli_exprdup_final` stays green (`/tmp/stage2_cli_exprdup_final_build.log`), and both fresh carriers `/tmp/stage_hello.cr` and `src/stdlib/prelude.cr --no-prelude` now reach `[RUNPROBE] 6b` and exit with the shared later `error: End of file reached` (`/tmp/stage2_cli_exprdup_final_hello.log`, `/tmp/stage2_cli_exprdup_final_prelude.log`) instead of the old early `Array(ExprId)#unsafe_fetch` segfault family. The decisive adversary split refutes the narrower `dup-only` theory: rebuilding from a branch that kept only `cached.roots.dup` / `parse_program_roots.dup` but removed deferred require loading regressed both carriers to `SIGSEGV` under the same fresh ladder (`/tmp/stage2_cli_exprdup_minimal_hello.log`, `/tmp/stage2_cli_exprdup_minimal_prelude.log`, `/tmp/stage2_cli_exprdup_minimal_hello_lldb.txt`). Independent `CRYSTAL_V2_STOP_AFTER_PARSE=1` vs `CRYSTAL_V2_STOP_AFTER_HIR=1` runs on the earlier full branch `/tmp/stage2_exprdup` further split the frontier: parse-only is green, HIR-stop is red, so the next live root-cause corridor is `entry.roots` consumption during HIR/top-level setup rather than parser-only require scanning. Reusable lesson: when a self-hosted `Array(ExprId)` crash moves under a small hardening branch, test the minimal split both ways before shrinking the patch; in this corridor, delayed recursion and immediate root duplication together form the stable boundary. {F/G/R: 0.95/0.78/0.97} [verified]
...
- [LM-hir-top-level-enum-name-recovery-2026-04-02]: the remaining HIR enum shorthand failures were not a late call-replay bug. The decisive trace on `trace :sched, foo: 1` showed `reorder_named_args(...)` was already invoking `lower_arg_with_expected_type(...)`, but enum coercion still missed because `@enum_info` contained no top-level `Section` entry at coercion time; a direct host-side probe against `register_enum(...)` confirmed `@enum_info.keys` stayed empty after registering top-level `enum Section`, while the namespaced control case produced `Crystal::Tracing::Section`. The bounded fix in `src/compiler/hir/ast_to_hir.cr` is to let `enum_name_from_node(...)` fall back to the parser-provided `node.name` even when no source text is attached, instead of returning early on `source.nil?`. With that metadata restored, both top-level enum shorthand coercion and enum `.to_i` / `.value` lowering become green again. Adversary / stale-spec split: the old expectation that `Signal::CHLD.reset` lowers as a member-singleton target was stale; current HIR intentionally resolves enum member method calls as ordinary instance dispatch on the enum value (`Signal#reset`). Verification: direct host-side `register_enum(...)` probe now prints `top: Section` and `qualified: Crystal::Tracing::Section`; focused `spec/hir/ast_to_hir_spec.cr` examples for `enum symbol arguments`, `enum literal to_i/value`, and `enum value method calls` are green; full `ast_to_hir_spec` moves from `4` failures to `2`, and `../crystal/bin/crystal build src/crystal_v2.cr --no-codegen --error-trace` stays green. Boundary: this closes the source-less top-level enum metadata corridor in HIR tests but does not address the two remaining real HIR failures around module-mixin `self` return typing and macro-body `extend self` registration. {F:0.98/G:0.86/R:0.99} [verified]
- [LM-hir-module-macro-return-cleanup-2026-04-02]: after [LM-hir-top-level-enum-name-recovery-2026-04-02], the two remaining HIR failures were not independent parser regressions. The decisive split came from exact focused examples: `macro if in module bodies registers extend self methods from macro bodies` stayed red even when macro lookup succeeded, and `module mixin return inference prefers concrete self type` still inferred the body as `Box` while the final annotated return collapsed back to module `M`. The bounded HIR fixes in `src/compiler/hir/ast_to_hir.cr` are: (1) treat `BeginNode` the same as `BlockNode` inside `register_module_members_from_macro_expansion(...)`, so macro-expanded module bodies no longer skip `extend self` and nested `def` registration; and (2) for module-like return annotations, prefer `infer_concrete_return_type_from_body(...)` before falling back to `annotation_type_ref(...)` during both initial and merged annotation resolution, so mixin-style instance returns stay concrete. The stale HIR expectation for enum member method calls was updated from `Signal::CHLD.reset` to the current lowering shape `Signal#reset`, and old unconditional debug prints around module-member registration / `mark_module_extend_self(...)` were removed. Adversary then falsified two tempting follow-up branches: a broad lazy-enum discovery layer that reparsed current/cached/loaded sources on misses was not required for the green HIR pack and correlated with whole-program RSS blow-up, so it was removed; and a broad `safe_slice_to_string(...)` hardening attempt was refuted because it turned `ast_to_hir_spec` massively red. The bounded final crash fix is instead `unary_operator_text(...)`, which recovers unary operators from source span (`node.span ... operand.span`) before falling back to the raw operator slice, eliminating the stage3 `SIGSEGV` on unreadable unary operator slices. Verification: focused `ast_to_hir_spec` examples for enum shorthand, enum `.to_i/.value`, macro-body `extend self`, module-like return typing, and unreadable unary slices are green; full `../crystal/bin/crystal spec spec/hir/ast_to_hir_spec.cr --error-trace` reports `121 examples, 0 failures, 0 errors, 2 pending`; `../crystal/bin/crystal build src/crystal_v2.cr --no-codegen --error-trace` stays green; rebuild of `/tmp/crystal_v2_hir_stage3probe` is green; and the full safe bootstrap log `/tmp/stage3_link_probe_after_unary_fix_debug.log` reports `semantic_diags=0 resolution_diags=0 type_diags=0` with `[EXIT: 0]` after ~207s under `scripts/run_safe.sh`. Boundary/adversary: the old `8192MB` watchdog kill was a misleading resource boundary, not the final blocker; the real remaining risk was unreadable unary operator slices during HIR lowering, and the bounded source-span fallback closes that corridor without reopening the refuted broad slice-hardening branch. {F:0.99/G:0.89/R:0.99} [verified]
- [LM-phase0-body-infer-identity-review-2026-04-02]: the open review complaint that phase0 `body_infer` metrics still keyed on caller arenas or a non-injective mixed integer surrogate does not reproduce on the current tree. The decisive code anchors are `src/compiler/hir/ast_to_hir.cr:10141`, where `canonical_def_identity_for_body_infer(...)` canonicalizes through `canonical_arena_for_def(...)` and returns a structured `CrystalV2::Compiler::Semantic::DefIdentity{arena_id, expr_index}`, and `src/compiler/hir/ast_to_hir.cr:14012`, where `infer_concrete_return_type_from_body(...)` computes final `resolved_arena` before calling `record_phase0_body_infer_walk(...)`. The docs are aligned too: `docs/phase0_contracts.md` already documents `lower_def_calls`, `lower_def_dupes`, `body_infer_walks`, `unique_defs`, and `body_infer_dupes`, and now explicitly notes that caller/preferred arenas are normalized away before counting. Focused regression `spec/hir/phase0_body_infer_metrics_spec.cr` is green and now includes a three-arena falsifier where the same reparsed `def` is recorded once via its canonical arena even when the walk is invoked from a wrong caller arena; `../crystal/bin/crystal build src/crystal_v2.cr --no-codegen --error-trace` stays green. Boundary/adversary: this is a review-hardening/documentation checkpoint, not a production behavior change; it closes the stale identity-key reopening risk but does not by itself advance any later runtime/link/bootstrap frontier beyond the already-green `stage3` baseline. {F:0.98/G:0.90/R:0.99} [verified]
- [LM-phase2-provenance-boundary-freeze-2026-04-02]: after the stage3-green HIR checkpoint and the closed phase0 review branch, the next honest post-bootstrap task was not more lowering surgery but the Phase 2.1 compile-shadow provenance contract. The decisive local seam was the overlapping helper layer in `src/compiler/semantic/compile_shadow_aggregate.cr`: `diagnostic_provenance_context_for(expr_id)`, the redundant `generated_diagnostic_context_for(expr_id)` alias, `format_shadow_diagnostic(...)` re-extracting node ids inline, and `src/compiler/cli.cr` reaching into raw `aggregate.provenance_for(node_id)` just to classify generated diagnostics in summary counts. The bounded Step A fix is behavior-preserving: add diagnostic-facing overloads of `diagnostic_provenance_context_for(...)`, add `generated_shadow_diagnostic?(...)` so generated-classification stays inside the aggregate boundary, remove the redundant alias, route `format_shadow_diagnostic(...)` through the diagnostic-facing seam, and make `cli.cr` use `generated_shadow_diagnostic?(...)` instead of direct `provenance_for(...)` probes. Focused boundary coverage is green: `spec/semantic/compile_shadow_aggregate_spec.cr` examples `exposes parsed provenance without generated metadata`, `adds origin note for generated type diagnostics`, and `adds macro definition note for cross-file generated type diagnostics`; `spec/semantic_cli_spec.cr` example `reports generated diagnostics inside macro-expanded class bodies` also stays green, and `../crystal/bin/crystal build src/crystal_v2.cr --no-codegen --error-trace` remains green. Boundary/adversary: the full `spec/semantic/compile_shadow_aggregate_spec.cr` pack is not a valid DoD for this checkpoint because it still has pre-existing generated-resolution red cases that fail before this new provenance seam is exercised; the honest claim is only that the diagnostic-facing Step A boundary is now in place and CLI-side provenance glue is smaller. {F:0.95/G:0.83/R:0.98} [verified]
- [LM-phase2-provenance-raw-rendering-2026-04-02]: after [LM-phase2-provenance-boundary-freeze-2026-04-02], the next remaining compile-shadow glue was not in generated provenance lookup itself but in diagnostic materialization. `src/compiler/cli.cr` still eagerly mapped parse/semantic/resolution/type diagnostics through `aggregate.enrich_shadow_diagnostic(...)` before parity, counting, and verbose printing, even though `format_shadow_diagnostic(...)` inside `src/compiler/semantic/compile_shadow_aggregate.cr` was already the intended aggregate-owned render path. The bounded follow-up is behavior-preserving: keep `shadow_*_diagnostics` raw in `cli.cr`, and make `format_shadow_diagnostic(...)` explicitly enrich raw frontend/semantic diagnostics before applying generated provenance context and selecting formatter sources. Focused regression coverage already existed for the decisive contract and is green: `spec/semantic/compile_shadow_aggregate_spec.cr` examples `formats parsed diagnostics without prior manual enrichment`, `formats semantic diagnostics without prior manual enrichment`, `adds origin note for generated type diagnostics`, and `adds macro definition note for cross-file generated type diagnostics`; CLI-side generated type rendering examples `prints macro definition note for cross-file generated type diagnostics` and `does not print redundant macro definition note for same-file generated type diagnostics` are also green, and `../crystal/bin/crystal build src/crystal_v2.cr --no-codegen --error-trace` remains green. Boundary/adversary: this closes the double-materialization seam on the render path, but the generated-resolution CLI/class-body example remains a separate pre-existing red frontier and should not be misread as fallout from this refactor. {F:0.96/G:0.84/R:0.98} [verified]
- [LM-phase2-provenance-summary-counts-2026-04-02]: after [LM-phase2-provenance-raw-rendering-2026-04-02], the next remaining compile-shadow summary glue was no longer raw diagnostic enrichment but unit-level counting. `src/compiler/cli.cr` still owned four local helpers for semantic/resolution/generated per-unit diagnostic counts even though all of them depended only on `CompileShadowAggregate` knowledge: `unit_index_for(...)`, generated provenance classification, and aggregate unit ownership. The bounded behavior-preserving move is to add `diagnostic_counts_by_unit(...)` and `generated_diagnostic_counts_by_unit(...)` overloads to `src/compiler/semantic/compile_shadow_aggregate.cr` for both frontend and semantic diagnostics, then switch `cli.cr` summary assembly to those aggregate-owned counters and delete the local counting helpers. Focused aggregate coverage is green: `spec/semantic/compile_shadow_aggregate_spec.cr` examples `tracks shadow reparse diagnostics per unit` and `reports type diagnostics inside generated top-level def bodies`; focused CLI summary coverage is also green: `spec/semantic_cli_spec.cr` examples `reports compile and shadow parse diagnostics separately in semantic shadow summaries`, `keeps strict semantic shadow green when parse parity matches`, and `reports generated type diagnostics separately in semantic shadow summaries`; `../crystal/bin/crystal build src/crystal_v2.cr --no-codegen --error-trace` stays green. Boundary/adversary: this narrows the remaining summary/parity glue in `cli.cr`, but it does not change parser diagnostic identities or the separate generated-resolution frontier, which remains outside this checkpoint. {F:0.96/G:0.85/R:0.98} [verified]
- [LM-phase2-provenance-summary-rows-2026-04-02]: after [LM-phase2-provenance-summary-counts-2026-04-02], the next smallest compile-shadow summary seam was no longer counting or raw rendering but the per-unit row assembly that still lived in `src/compiler/cli.cr`. The decisive local smell was the duplicated `ensure_shadow_unit_metric_size!` / `shadow_unit_metric!` pair plus the `aggregate.unit_summaries.each_with_index` loop that manually threaded eleven per-unit metric arrays together with aggregate-owned `generated_root_count_for_unit`, `generated_node_count_for_unit`, and `owned_node_count_for_unit` just to instantiate `SemanticShadowUnitSummary`. The bounded behavior-preserving move is to let `src/compiler/semantic/compile_shadow_aggregate.cr` expose `summary_unit_metrics(...)`, keep the size guards there with the same label-specific failure messages, and return aggregate-owned summary rows for CLI to wrap into `SemanticShadowUnitSummary`. Focused aggregate coverage is green: `spec/semantic/compile_shadow_aggregate_spec.cr` examples `builds per-unit summary rows from aggregate-owned and raw metric counts` and `raises when per-unit summary metrics do not match aggregate unit count`; focused CLI summary coverage remains green on `reports compile and shadow parse diagnostics separately in semantic shadow summaries`, `keeps strict semantic shadow green when parse parity matches`, and `reports generated type diagnostics separately in semantic shadow summaries`; `../crystal/bin/crystal build src/crystal_v2.cr --no-codegen --error-trace` stays green. Adversary/boundary: the existing `reports generated resolution diagnostics separately in semantic shadow summaries` example is still red with `generated_resolution_diags=0`, so this checkpoint must not be overstated as a fix for generated-resolution accounting; it only removes the row-assembly glue and keeps the known frontier explicit. {F:0.97/G:0.85/R:0.98} [verified]
- [LM-phase2-generated-resolution-method-body-suppression-2026-04-02]: after [LM-phase2-provenance-summary-rows-2026-04-02], the apparent remaining Phase 2.1 issue was not summary counting or CLI row assembly anymore but a real semantic resolver frontier. The decisive falsifier chain showed that both aggregate and CLI shadow paths still produced `generated_resolution_diags=0 generated_type_diags=1` for macro-expanded method/class bodies with `missing + 1`, while the long-standing bare-callee resolver guard `spec/semantic/name_resolver_spec.cr` expected `def run; missing; end` to stay quiet. Exact probes then localized the gap: `src/compiler/semantic/resolvers/name_resolver.cr` suppressed every lowercase unresolved identifier inside method bodies through `suppress_unresolved_callee_diagnostic?`, even when the identifier was nested inside a larger expression rather than acting as a bare deferred implicit-call candidate. The bounded fix is to track bare statement-root identifiers separately and keep suppression only for true deferred callee candidates: explicit callees while `@call_callee_depth > 0`, plus standalone identifier statements visited in method-body expression lists. Nested unresolved identifiers like `missing + 1` now emit resolution diagnostics again, including generated shadow diagnostics in macro-expanded top-level def bodies and class bodies. Focused verification is green: `spec/semantic/name_resolver_spec.cr` examples `defers unresolved bare callee candidates inside method bodies` and `emits diagnostics for unresolved identifiers nested inside method-body expressions`, full `spec/semantic/name_resolver_spec.cr`, aggregate examples `reports resolution diagnostics inside generated top-level def bodies`, `reports resolution diagnostics inside generated top-level class bodies`, and `adds macro definition note for cross-file generated resolution diagnostics`, CLI examples `reports generated resolution diagnostics separately in semantic shadow summaries`, `reports generated diagnostics inside macro-expanded class bodies`, and `prints macro definition note for cross-file generated diagnostics`, plus `../crystal/bin/crystal build src/crystal_v2.cr --no-codegen --error-trace`. Boundary/adversary: this closes the explicit generated-resolution frontier in semantic shadow summaries, but it does not yet remove the remaining top-level summary object construction glue from `cli.cr`; the next honest Phase 2.1 seam returns to summary assembly/render ownership now that the red generated-resolution branch is no longer masking it. {F:0.98/G:0.87/R:0.99} [verified]
- [LM-stage4-selfhosted-lexer-families-2026-04-02]: the red `stage4` bootstrap/perf branch after green `stage3` was not one broad RC-elision regression. Exact no-prelude reducers split it into two concrete self-hosted lexer no-progress families plus a separate parser-debug hot path. First, tiny string inputs (`/tmp/lexer_string.cr`, `/tmp/lexer_string_escape.cr`) showed endless zero-length string tokens until `src/compiler/frontend/lexer.cr#lex_string` stopped depending on omitted `advance(count = 1)` in the active self-hosted path: raw cursor movement in the no-escape fast path, explicit `advance(1)` in the processed/interpolation subpaths. Second, prelude `hello` memdiag no longer pointed at `lex_string`; exact reducers `/tmp/lexer_symbol_only.cr`, `/tmp/lexer_colon_colon.cr`, and `/tmp/lexer_type_decl.cr` then proved a second no-progress family in `lex_symbol_or_colon` / `try_lex_operator_symbol(...)`, again from omitted `advance`, with token-preload traces stuck on zero-length kind `107` slices at fixed offsets. Converting that colon/symbol corridor to explicit `advance(1)` closes the second runaway as well. In parallel, prelude `hello` timeout samples showed our own `debug: trace tiny token preload fill during parser init` hook had become a real hot-path pollutant: `Parser#trace_token_preload_enabled?` called `BootstrapEnv.enabled?("CRYSTAL_V2_TRACE_TOKEN_PRELOAD")` on every token. Caching `@trace_token_preload_enabled` once in parser initialization removes `getenv` from the hot loop. Focused verification is green: `spec/lexer/lexer_spec.cr`, `../crystal/bin/crystal build src/crystal_v2.cr --no-codegen --error-trace`, and rebuilt self-hosted artifacts from `/tmp/crystal_v2_hir_stage3probe` report bounded stage4 rebuild totals (`242585.3ms`, then `242385.4ms`). Exact reducers now terminate token preload with finite token shapes instead of multi-million loops. Adversary/boundary: after these fixes the remaining fronts are bounded and different: no-prelude runs now abort on downstream runtime heads (`Array#empty?`, `Time::Span#to_i`, `LibMachVM.mach_task_self` via `safe_slice_to_string`), and prelude `hello` no longer blows memory but crashes immediately in `Parser#slice_eq(...)` from `parse_identifier_like`. Reusable lesson: for self-hosted lexer regressions, do not reopen generic RC theories first; split the branch by token family with tiny no-prelude reducers, and treat omitted default-arg call sites plus debug env hooks as separate, falsifiable corridors. {F:0.98/G:0.83/R:0.99} [verified]
- [LM-454|verified]: The crashing `Time::Span` / integer-print corridor was not
  a stdlib mismatch and not a single bad LLVM override; it was an abstract
  `Int` ABI seam in `src/compiler/mir/llvm_backend.cr#emit_call(...)`. The
  decisive reducer `puts 5.remainder(3); puts 5.tdiv(3); puts
  Time::Span.new(seconds: 0, nanoseconds: 123).nanoseconds` compiled cleanly
  but initially crashed in `Int#remainder(Int32)` because abstract `Int`
  instance calls still treated concrete scalar receivers as packed-scalar
  `ptr self`, while lowered bodies mixed `load %self`, `ptrtoint %self`, and
  `inttoptr result`. The bounded backend repair is threefold: (1) bypass
  abstract `Int#remainder(Int32)` on concrete scalar receivers and emit the
  checked remainder directly; (2) bypass abstract `Int#tdiv(Int32)` likewise,
  but return a slot pointer while preserving the concrete integer logical type,
  which lets later consumers either load the quotient or widen it numerically;
  and (3) bypass abstract `Int#to_s(...)` / `Int#to_s(IO, ...)` for concrete
  scalar receivers by dispatching directly to concrete integer `#to_s`
  implementations (IO path via concrete `#to_s` plus `IO#<<(String)`). Focused
  backend specs for abstract `remainder`, `tdiv`, and both `to_s` variants are
  green; the tiny runtime reducer is green under `scripts/run_safe.sh`; and the
  real benchmark compile gates are green for
  `/Users/sergey/Projects/Python/Grafana/python/bench_crystal.cr` and
  `examples/bench_comprehensive.cr`. The runtime heads have moved forward into
  separate non-Int bugs: `bench_comprehensive` now reaches `Primes below
  5000000: 348513` before crashing in
  `Crystal::PointerLinkedList(Channel::Receiver(Int32))#empty?`, and
  `bench_crystal` now crashes later in `__vdispatch__Object#to_s(IO)` from the
  old `sprintf("%.3f", value)` frontier. Boundary: LM-454 closes the abstract
...
- [LM-455|verified]: The `sprintf("%.10f", ...)` / Ryu precision corridor was
  ...
  [verified]
- [LM-hir-system-time-shorthand-2026-04-14]: Forked stdlib `System::Time` is a shorthand for `Crystal::System::Time` only inside nested `Crystal::*` contexts, not at top level. The root AST shape for `System::Time.instant` is `MemberAccessNode` with a `PathNode` receiver, so dotted-Identifier fixes and MIR aliases miss the real path and can mask resolver bugs. Commit `e4502e79` handles the `PathNode` static receiver in HIR, includes the current HIR function owner in shorthand-context checks, resolves valid stdlib shorthand to `Crystal::System::Time.instant`, and rejects top-level `System::Time.instant` with a compile failure. Verification: `/tmp/cv2_system_time_guard` build is green; `system_time_shorthand_unpack_repro` runs and prints `system_time_shorthand_unpack_ok`; `system_time_shorthand_top_level_negative_repro.sh` reports ok; emitted HIR shows `Crystal::System::Time.instant` followed by `field_get @@seconds` / `@@nanoseconds`; `bench_fiber_yield_sleep_path_repro` prints `before_yield` / `after_yield`; adjacent lib-struct and block-overload regressions are green. Boundary: `examples/bench_comprehensive.cr` compiles, but runtime still segfaults later after the primes phase, so the next frontier is not the `System::Time` shorthand/fiber-yield stub family. {F:0.96/G:0.82/R:0.98} [verified]
- [LM-hir-allocator-state-channel-2026-04-14]: The `examples/bench_comprehensive.cr` crash after `Primes below 5000000: 348513` was not a remaining `System::Time`/Fiber-yield issue and not only the earlier `ptr.value.field=` write bug. Two bugs stacked. First, the write-side HIR fast path for `ptr.value.field = value` treated every `Pointer(T)` as inline struct bytes; for V2 heap-backed structs/classes it must `PointerLoad` the object pointer before `FieldSet`, while only C/lib structs are inline. This closed the exact `PointerLinkedList#push` corruption in `regression_tests/complex/test_channel_receive_state.cr`. Second, `Channel(Int32).new` allocator bodies were generated, later removed by generic-base layout invalidation (`strip_generic_args("Channel(Int32)") == "Channel"`), but allocator bookkeeping only invalidated `Channel`, leaving `@generated_allocators` falsely claiming `Channel(Int32)` still had a body. LLVM then synthesized a fallback allocator `calloc(4096); ret`, leaving `@senders` / `@receivers` null. The verified fix keeps concrete generic allocator bookkeeping in sync with body invalidation, refuses to trust `@generated_allocators` when the body is missing, pre-lowers `initialize` before freezing allocator layout, and runs a final bounded deferred-allocator flush after late pending/safety-net lowering. Verification: `/tmp/cv2_allocator_state_fix` builds green; emitted LLVM for `bench_comprehensive` has `define ptr @Channel$LInt32$R$Dnew(i32 %capacity)` storing initialized `@senders` and `@receivers`, with callsites passing `i32 0`; `scripts/run_safe.sh /tmp/bench_comprehensive_state_fix 30 512` prints through `Ping-pong`, `Fibers`, `Done.` and exits 0; adjacent `test_channel_receive_state`, `ptr_value_field_heap_struct_repro`, and `System::Time` shorthand regressions are green. Boundary: `/Users/sergey/Projects/Python/Grafana/python/bench_crystal.cr` compiles, but runtime now exposes the separate stub `Indexable$LTuple$LFloat64$R$R$Hsize` under a 4GB safe-run limit. {F:0.97/G:0.80/R:0.98} [verified]
- [LM-hir-int-upto-downto-block-2026-04-14]: `sprintf("%.3f", 1.25_f64)` / direct `Float::Printer::RyuPrintf.d2fixed_buffered_n(1.25_f64, 3, ...)` lost the integer prefix because Ryu's `(len - 1).downto(0) { ... }` block body was not executed by the generic block/proc fallback for stdlib `Int#downto`. The bounded fix lowers `Int#upto(to) { ... }` and `Int#downto(to) { ... }` as caller-frame HIR loops before block materialization, with caller-local PHIs and the same post-yield `x < to` / `x > to` guard as stdlib to avoid an extra overflowing step at single-value boundaries. Verification: `/tmp/cv2_int_iter_review` builds green; `scripts/run_safe.sh /tmp/int_downto_block_overload_review 5 512` prints `int_downto_block_overload_ok`; `scripts/run_safe.sh /tmp/sprintf_float_fixed_prefix_review 5 512` prints `sprintf_float_fixed_prefix_ok`; `scripts/run_safe.sh /tmp/bench_comprehensive_review 60 2048` prints through `Done.` and exits 0. Boundary: `bench_comprehensive` still has wrong channel/fiber aggregate values, and external `bench_crystal.cr` still exits `133` under `run_safe` despite compiling. {F:0.94/G:0.72/R:0.96} [verified]
- [LM-hir-hash-new-return-pin-2026-04-19]: The `test_edge_hash_complex` segfault after the `Hash#each` writeback fix was not runtime memory corruption. Emitted HIR showed that after `Hash#each` forced constructor helper lowering, the subsequent top-level `Hash(String, Int32).new(0)` call was typed as `Hash(String, Void)`, so later `[]=` / `[]` dispatched through the wrong specialization and eventually called `Object#to_s` on integer payload `0x1`. The bounded fix pins concrete class `new` return types after owner resolution in `src/compiler/hir/ast_to_hir.cr`, preventing the late "prefer actual lowered function return type" repair from downgrading constructor call-sites to a helper body's actual lowered return type. Verification: `crystal build src/crystal_v2.cr -o bin/crystal_v2 --error-trace` is green with only the known `Random::DEFAULT` warning; `regression_tests/hash_each_then_hash_new_type_repro.sh bin/crystal_v2`, `hash_each_block_writeback_repro.sh`, and `hash_small_linear_scan_repro.sh` are green; focused HIR for `/tmp/hash_after_each_empty_counts.cr` now keeps the post-`Hash#each` constructor and following `[]=`/`[]` calls on `Hash(String, Int32)`; `regression_tests/combined/test_edge_hash_complex.cr` prints `hash_complex_all_ok`; and `regression_tests/run_combined.sh bin/crystal_v2 4` moves the combined suite to `28 passed, 3 failed out of 31`. Boundary: this closes the hash constructor type-pollution family, not the remaining combined-suite generic dispatch, generics/unions, or strings/join failures. {F:0.96/G:0.78/R:0.98} [verified]
- [LM-hir-nilable-init-param-forwarding-2026-04-19]: The `test_nilable_struct_union_layout` segfault was not an LLVM layout issue after the stored class metadata was inspected. `DEBUG_INIT_PARAMS=NodeWithNilableStructs` showed `@init_params` already held the declared nilable ivar types (`Array(Int32)?`, `Slice(UInt8)?`, `Bool?`), but emitted HIR showed the shared `NodeWithNilableStructs#initialize$arity7` body had been lowered from the first non-nil constructor call as `Array/Slice/Bool` params. A later `.new(..., nil, nil, nil, nil, nil)` overload then passed `Nil` into those non-nil slots and the initializer wrapped them as non-nil union variants before crashing. The bounded fix separates allocator wrapper parameter types from the initializer forwarding signature: `allocator_initializer_param_types(...)` preserves declared nilable union init params, and allocator wrappers coerce forwarded args with `UnionWrap` before calling `initialize`. Verification: `crystal build src/crystal_v2.cr -o bin/crystal_v2 --error-trace` is green with only the known `Random::DEFAULT` warning; focused HIR now has `NodeWithNilableStructs#initialize$arity7(..., Nil | Array(Int32), Nil | Slice(UInt8), Nil | Bool, ...)` and both nil/non-nil constructor overloads emit `union_wrap` before the call; the focused binary prints `layout_ok` under `scripts/run_safe.sh`; `stage2_nilable_struct_union_overflow_repro.sh` reports the layout bug not reproduced; `run_combined.sh bin/crystal_v2 4` remains `31 passed, 0 failed`; and `run_all.sh bin/crystal_v2` moves to `141 passed, 4 failed out of 145`. Boundary: this closes the allocator/initializer nilable forwarding crash, not the separate `complex/test_nilable_struct_union` loop-local writeback bug where `count += 1 unless ...` computes an increment but the loop-back phi keeps the old count. {F:0.97/G:0.78/R:0.98} [verified]
- [LM-hir-byteformat-decode-u32-2026-04-19]: The `test_byteformat_decode_u32` abort was a stacked HIR/MIR/LLVM boundary bug, not only a missing `IO::ByteFormat::LittleEndian#==` method. First, `SystemEndian == self` inside endian protocol bodies lowered module/type-literal equality as a normal instance call, producing the missing `IO::ByteFormat::LittleEndian#==(IO::ByteFormat::LittleEndian)` stub. The bounded HIR fix folds `==`, `!=`, and `===` for type/module literal operands before method dispatch, so focused HIR now emits `literal true : Bool` for the LittleEndian checks and contains no `ByteFormat::LittleEndian#==` call. Second, once the endian branch executed, `__vdispatch__IO#read(Slice)` passed a union alloca pointer into concrete `IO::Memory#read(Slice(UInt8))`, corrupting `Slice#copy_from`; the backend now unwraps the union payload when the callee parameter is concrete pointer-shaped, while preserving full tag+payload storage for union parameters. Third, stdlib `IO#read_fully(Slice(UInt8))` currently lowers through a polluted `slice += read_bytes` HIR loop that can call `Int32#size`; a localized LLVM override performs the read loop directly and registers its synthetic callees so compile units without the vdispatch body still produce valid IR. Verification: `crystal build src/crystal_v2.cr -o bin/crystal_v2 --error-trace` is green with only the known `Random::DEFAULT` warning; `test_byteformat_decode_u32` compiles and prints `byteformat_u32_ok` under `scripts/run_safe.sh`; focused HIR confirms the literal endian check shape; `run_mini_oracles.sh bin/crystal_v2` reports `6 passed, 0 failed`; `run_combined.sh bin/crystal_v2 4` remains `31 passed, 0 failed`; and `run_all.sh bin/crystal_v2` moves to `142 passed, 3 failed out of 145`. Boundary: this closes the byteformat `UInt32` decode/runtime path, but not the remaining `sprintf_float_precision` (`case6`), `test_3mod` (`Type#name` stub), or `test_closure_ref` exit-138 fronts. {F:0.96/G:0.76/R:0.98} [verified]
- [LM-hir-unless-branch-local-writeback-2026-04-19]: The remaining `sprintf_float_precision` `case6` failure after the byteformat checkpoint was not another formatter/Float parser bug. A direct `Float::Printer::RyuPrintf.d2fixed_buffered_n(0.1_f64, 5, buf)` probe returned `.10000`; emitted HIR showed the `unless nonzero` branch wrote `'0'` at `result[index]` and computed `index &+= 1`, but the merge block still fed the old index into the decimal-point write, so `'.'` overwrote the leading zero. The bounded HIR fix captures `lower_unless` then-branch locals before `ctx.pop_scope`, matching the already-correct else-branch ordering. Verification: `crystal build src/crystal_v2.cr -o bin/crystal_v2 --error-trace` is green with only the known `Random::DEFAULT` warning; `regression_tests/unless_branch_local_writeback.cr` prints `unless_branch_local_writeback_ok` under `scripts/run_safe.sh`; `regression_tests/sprintf_float_precision.cr` prints `sprintf_float_precision_ok`; the direct Ryu probe now prints `0.10000`; `run_mini_oracles.sh bin/crystal_v2` reports `6 passed, 0 failed`; `run_combined.sh bin/crystal_v2 4` remains `31 passed, 0 failed`; and `run_all.sh bin/crystal_v2` moves to `144 passed, 2 failed out of 146`. Boundary: this closes the `unless` local-writeback/Ryu leading-zero corridor, but not the remaining `test_3mod` (`Type#name` stub) or `test_closure_ref` exit-138 fronts. {F:0.97/G:0.78/R:0.98} [verified]
- [LM-hir-abstract-module-self-dispatch-2026-04-19]: The `test_3mod` `STUB CALLED: Type#name` failure was not a missing concrete `IntType#name` implementation and not a safe reason to broadly virtualize every missing self call. Focused HIR/MIR showed `Type#describe` lowered receiverless `name` and `byte_size` as static calls to abstract `Type#name` / `Type#byte_size`, even though the contracts came from included abstract modules `Named` and `Sized` and concrete subclasses implemented them. The bounded fix is only in `lower_identifier`: inside non-struct instance methods, a zero-arg receiverless identifier whose name is an abstract def on an included module is emitted as a virtual self call, with concrete target lowering and inferred virtual return type. Verification: `crystal build src/crystal_v2.cr -o bin/crystal_v2 --error-trace` is green with only the known `Random::DEFAULT` warning; `regression_tests/test_3mod.cr` prints `Int32(4)`, `Ptr(Int32)(8)`, `Arr(Int32)(24)`, and `done` under `scripts/run_safe.sh`; adjacent `test_6_classes`, `test_module`, and `test_complex_hierarchy` are green; `run_mini_oracles.sh bin/crystal_v2` reports `6 passed, 0 failed`; `run_combined.sh bin/crystal_v2 4` remains `31 passed, 0 failed`; and `run_all.sh bin/crystal_v2` moves to `145 passed, 1 failed out of 146`. Boundary: this closes the abstract-module implicit-self dispatch corridor, not the remaining `test_closure_ref` closure-cell/ABI front. {F:0.97/G:0.74/R:0.98} [verified]
- [LM-hir-closure-ref-heap-block-proc-2026-04-19]: The `test_closure_ref` bus error was not solved by changing Proc call lowering alone. Focused LLVM showed `call_block(&block)` received a raw block function pointer while the callee-side `Proc#call` path already expected a heap Proc object and loaded `{fn, env}` from offsets `0` and `8`, so it treated `@__crystal_block_proc_1` code bytes as Proc object storage. After heap-materializing block arguments only when the target def actually calls its block parameter, the runtime no longer crashes, but the first attempt still printed `10` because `pop_scope` restored `@boxed_locals` after inline block-argument materialization and hid the box from the parent `puts x`. The bounded HIR fix adds `block_arg_requires_heap_proc?`, extends `def_contains_block_call?` with a source-span `block.call` fallback for bodies missed by AST traversal, uses that predicate at the late call, inline-yield fallback, and inline block-param binding sites, and keeps boxes visible to their owner local across same-function scope restores. Verification: `regression_tests/test_closure_ref.cr` compiles and prints `42` under `scripts/run_safe.sh`; adjacent `test_proc_basic`, `test_blocks`, and `test_blocks_closures` are green; `run_mini_oracles.sh bin/crystal_v2` reports `6 passed, 0 failed`; repeated `run_combined.sh bin/crystal_v2 4` reports `31 passed, 0 failed` after a non-reproduced transient `test_control_flow` `ExprId out of bounds`; and `run_all.sh bin/crystal_v2` reports `146 passed, 0 failed out of 146`. Boundary: this closes the current full-suite closure-ref red without claiming the larger closure-env ABI/P1 atomic flip is complete. {F:0.97/G:0.72/R:0.98} [verified]
- [LM-hir-owner-aware-boxed-locals-2026-04-19]: Plain name-keyed monotonic boxed-local tracking was too broad after heap Proc block-argument materialization. The two-probe spawn guard first fixed direct block-param capture (`probe_block_param_ok`) but still failed helper-arg capture (`probe_helper_arg_FAIL sum=12`) because the second `4.times do |i|` in the same compile unit read the first probe's boxed `i`; focused LLVM showed `send_id` received a load from the stale first-loop box instead of the second loop index. The bounded fix adds `owner_local` to `LoweringContext::BoxedLocal` and routes boxed read/write/reuse through `lookup_boxed_local_for_local`, allowing reuse only for matching current lexical bindings, env-bound entries, or entry-required parent captures. `hoist_box_for_local` records the owning `initial_value`, and `capture_var_for_env` checks ownership before reusing a box. Verification: `spawn_capture_block_param_repro.sh bin/crystal_v2` now prints both `_ok` markers and exits `1` (fixed guard status); `test_closure_ref` still prints `42`; manual split two-probe output is `a=6` and `b=6`; adjacent Proc/block smokes are green; `run_mini_oracles.sh bin/crystal_v2` reports `6 passed, 0 failed`; and `run_combined.sh bin/crystal_v2 4` reports `31 passed, 0 failed`. Boundary: this fixes same-name boxed-local aliasing, not the remaining broader closure-env ABI cleanup or legacy `@closure_ref_cells` removal. {F:0.96/G:0.76/R:0.97} [verified]
- [LM-p1-ir-shape-guard-2026-04-19]: `regression_tests/p1_ir_shape_check.sh` is the current focused shape guard for the hybrid closure-env ABI boundary. It emits HIR and LLVM for a fixture that combines `call_block(&block)` with a zero-capture proc literal, dynamically extracts the captured `__crystal_block_proc_N` symbol from HIR, and verifies: boxed `make_closure` metadata is present, `make_proc` materializes Proc values, the focused heap `Proc#call` has no hidden capture args, LLVM does not use `%__crystal_proc` as a by-value Proc type, the captured block receives `ptr %__closure_env`, and heap Proc object storage/dispatch uses fn@0/env@8. Verification: `bash -n regression_tests/p1_ir_shape_check.sh` is clean; `regression_tests/p1_ir_shape_check.sh bin/crystal_v2` prints `p1_ir_shape_ok captured_fn=__crystal_block_proc_1` and exits 0; adjacent spawn/conditional/escaping closure forward guards still report fixed-state exit 1. Boundary: this is an additive guard for the present hybrid ABI shape, not proof that legacy `@closure_ref_cells` cleanup is complete. {F:0.90/G:0.65/R:0.92} [verified]
- [LM-p1-hybrid-boundary-guard-2026-04-19]: `regression_tests/p1_hybrid_boundary_guard.sh` is the current source-level guard for intentional pre-cleanup closure-env ABI anchors. After the dead `@proc_captures_by_value` cleanup, it now rejects reintroducing that side map and verifies the still-live `@closure_ref_cells` / `@closure_ref_prefer_cell` fallback anchors plus the dual-mode `lower_block_to_proc` split where heap paths return `emit_make_proc_value(...)` while legacy raw callback paths still use closure cells plus `FuncPointer`. It also checks that the plan/TODO/collab docs still mark this as a hybrid boundary. Verification: `bash -n regression_tests/p1_hybrid_boundary_guard.sh` is clean; `regression_tests/p1_hybrid_boundary_guard.sh` prints `p1_hybrid_boundary_ok` and exits 0; `regression_tests/p1_ir_shape_check.sh bin/crystal_v2` remains green. Boundary: this is presence-based and should be updated or removed together with the coupled ABI cleanup that intentionally deletes the remaining closure-cell anchors. {F:0.82/G:0.60/R:0.92} [verified]
- [LM-p1-dead-proc-capture-map-2026-04-19]: `@proc_captures_by_value` was dead after heap Proc materialization. `rg` found no initial producer assignment, only copy-propagation writes and two HIR `Proc#call` hidden-arg consumers, so the map could never become non-empty. The cleanup removed the map, propagation sites, and hidden-arg append checks, and updated `p1_hybrid_boundary_guard.sh` to reject reintroduction while continuing to guard live `@closure_ref_cells` / dual-mode `lower_block_to_proc` anchors. Verification: source `rg` has no `@proc_captures_by_value` matches in `ast_to_hir.cr`; build is green with only the known `Random::DEFAULT` warning; `p1_hybrid_boundary_guard.sh` and `p1_ir_shape_check.sh bin/crystal_v2` are green; spawn/conditional/escaping closure guards remain fixed-state green; `run_mini_oracles.sh bin/crystal_v2` reports `6 passed, 0 failed`; `run_combined.sh bin/crystal_v2 4` reports `31 passed, 0 failed`; and `run_all.sh bin/crystal_v2 4` reports `146 passed, 0 failed`. Boundary: this is not the closure-cell cleanup; `@closure_ref_cells` remains live for legacy raw block callbacks. {F:0.95/G:0.76/R:0.98} [verified]
- [LM-p1-raw-callback-shape-guard-2026-04-19]: `regression_tests/p1_raw_callback_shape_check.sh` pins the currently intentional legacy raw callback path. The focused fixture `def each_once(&block : Int32 ->); yield 3; end` with mutable captured `sum` prints `4`, and emitted HIR shows a `__closure_cell_N` plus bare `func_pointer @__crystal_block_proc_N`, with no focused `make_proc` / `make_closure`; the generated raw block function reads and writes the same closure cell and does not use `@__closure_env`. Verification: `bash -n regression_tests/p1_raw_callback_shape_check.sh` is clean; `regression_tests/p1_raw_callback_shape_check.sh bin/crystal_v2` prints `p1_raw_callback_shape_ok block_fn=__crystal_block_proc_1 cell=__closure_cell_2` and exits 0. Boundary: update this guard together with the behavior-changing ABI cleanup that intentionally replaces raw closure-cell callbacks. {F:0.90/G:0.68/R:0.94} [verified]
