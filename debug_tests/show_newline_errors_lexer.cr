...
puts "=== NEWLINE ERRORS IN LEXER.CR ==="
puts "Total: #{parser.diagnostics.count { |d| d.message.downcase.includes?("newline") }}\n"

parser.diagnostics.each_with_index do |diag, idx|
  next unless diag.message.downcase.includes?("newline")
