  ...
  module Compiler
    ...
    class CLI
      ...
      private class SemanticShadowSummary
        ...
        getter generated_resolution_diagnostic_count : Int32
        getter type_diagnostic_count : Int32
        getter generated_type_diagnostic_count : Int32
      ...
      struct Options
        ...
        # AST cache loading is currently disabled below because the serialized
        # AST roundtrip loses data in the compiler path. Keep cache writes
        # opt-in via --ast-cache so normal compiles do not pay save-only I/O.
          ...
          if shadow_summary
            ...
            if options.verbose || options.stats
              out_io.puts [
                ...
                "generated_resolution_diags=#{shadow_summary.generated_resolution_diagnostic_count}",
                "type_diags=#{shadow_summary.type_diagnostic_count}",
                "generated_type_diags=#{shadow_summary.generated_type_diagnostic_count}",
            ...
            if options.verbose
              ...
              shadow_summary.unit_summaries.each do |unit_summary|
                out_io.puts [
                  ...
                  "generated_resolution_diags=#{unit_summary.generated_resolution_diagnostic_count}",
                  "type_diags=#{unit_summary.type_diagnostic_count}",
                  "generated_type_diags=#{unit_summary.generated_type_diagnostic_count}",
                ].join(" ")
              end
            end
          end
      ...
      private def parse_file_recursive(
        ...
        if false && options.ast_cache
          ...
          if cached = LSP::AstCache.load(abs_path, source_mtime_ns)
            @ast_cache_hits += 1
            arena = cached.arena
            # Self-hosted release builds can mis-handle Array(ExprId) carriers
            # returned from parser/cache paths. Duplicate roots immediately so
            # downstream require scanning and lowering walk fresh GC-managed
            # storage instead of reusing the original backing buffer.
            exprs = cached.roots.dup
        ...
        base_dir = safe_dirname(abs_path)
        requires = [] of String
        # V2 stage2: require scanning is isolated into its own helper to shrink
      ...
      private def process_require_node(
        ...
        when Frontend::ModuleNode
          if body = node.body
            ...
            while body_i < body.size
              ...
              process_require_node(arena, child_id, source, base_dir, input_base_dir, options, out_io, requires_out)
              body_i += 1
            end
          end
        ...
        when Frontend::MacroIfNode
          ...
          if condition == true
            process_require_node(arena, node.then_body, source, base_dir, input_base_dir, options, out_io, requires_out)
          elsif condition == false
            if else_body = node.else_body
              process_require_node(arena, else_body, source, base_dir, input_base_dir, options, out_io, requires_out)
            end
          else
            process_require_node(arena, node.then_body, source, base_dir, input_base_dir, options, out_io, requires_out)
            if else_body = node.else_body
      ...
      private def scan_requires_from_exprs(
        ...
        while expr_i < expr_count
          expr_id = exprs.unsafe_fetch(expr_i)
          begin
            process_require_node(arena, expr_id, source, base_dir, input_base_dir, options, out_io, requires_out)
          rescue ex : IndexError
            if options.verbose
      ...
      private def parse_required_files(
      ...
      private def collect_top_level_nodes(
        ...
        when Frontend::MacroLiteralNode
          ...
          if has_control_flow
            ...
            if expanded = expand_macro_literal_via_expander(expr_id, arena, source, flags, macro_origin_path)
              if env_enabled?("DEBUG_MACRO_EXPAND")
      ...
      private def build_shadow_collector_declaration_inventory(
        ...
        i = 0
        while i < def_nodes.size
          entry = def_nodes.unsafe_fetch(i)
          inventory.record(
            Semantic::CompileShadowDeclarationKind::Methods,
            String.new(entry[0].name),
            shadow_collector_declaration_origin(entry[1], direct_arena_ids)
          )
          i += 1
        end
        ...
        i = 0
        while i < class_nodes.size
          entry = class_nodes.unsafe_fetch(i)
          inventory.record(
            Semantic::CompileShadowDeclarationKind::Classes,
            String.new(entry[0].name),
            shadow_collector_declaration_origin(entry[1], direct_arena_ids)
          )
          i += 1
        ...
        i = 0
        while i < module_nodes.size
          entry = module_nodes.unsafe_fetch(i)
          inventory.record(
            Semantic::CompileShadowDeclarationKind::Modules,
            String.new(entry[0].name),
            shadow_collector_declaration_origin(entry[1], direct_arena_ids)
          )
          i += 1
        ...
        while i < enum_nodes.size
          entry = enum_nodes.unsafe_fetch(i)
          inventory.record(
            Semantic::CompileShadowDeclarationKind::Enums,
            String.new(entry[0].name),
            shadow_collector_declaration_origin(entry[1], direct_arena_ids)
          )
          i += 1
        ...
        while i < macro_nodes.size
          entry = macro_nodes.unsafe_fetch(i)
          inventory.record(
            Semantic::CompileShadowDeclarationKind::Macros,
            String.new(entry.node.name),
            shadow_collector_declaration_origin(entry.arena, direct_arena_ids)
          )
          i += 1
        end
        ...
        while i < constant_exprs.size
          ...
          expr_id = entry[0]
          arena = entry[1]
          origin = shadow_collector_declaration_origin(arena, direct_arena_ids)
          node = arena[expr_id]
          case node
          when Frontend::ConstantNode
      ...
      private def shadow_collector_declaration_origin(
        arena : Frontend::ArenaLike,
        direct_arena_ids : Set(UInt64)
      ) : Semantic::CompileShadowDeclarationOrigin
        if direct_arena_ids.includes?(arena.object_id.to_u64)
      ...
      private def expand_shadow_collector_root_macro_invocation(
        ...
        expander = Semantic::MacroExpander.new(
          ...
          macro_source_path: macro_stat_path
        )
      ...
      private def run_semantic_compile_shadow(
        ...
        unit_summaries = aggregate.summary_unit_metrics(
          symbols_by_unit,
          generated_symbols_by_unit,
          identifiers_by_unit,
          compile_parse_diagnostics_by_unit,
          shadow_parse_diagnostics_by_unit,
          semantic_diagnostics_by_unit,
          generated_semantic_diagnostics_by_unit,
          resolution_diagnostics_by_unit,
          generated_resolution_diagnostics_by_unit,
          type_diagnostics_by_unit,
          generated_type_diagnostics_by_unit,
        ).map do |unit_summary|
          SemanticShadowUnitSummary.new(
            path: unit_summary.path,
            roots_count: unit_summary.roots_count,
            analysis_root_count: unit_summary.analysis_root_count,
            generated_root_count: unit_summary.generated_root_count,
            node_count: unit_summary.node_count,
            owned_node_count: unit_summary.owned_node_count,
            generated_node_count: unit_summary.generated_node_count,
            symbol_count: unit_summary.symbol_count,
            generated_symbol_count: unit_summary.generated_symbol_count,
            identifier_count: unit_summary.identifier_count,
            compile_parse_diagnostic_count: unit_summary.compile_parse_diagnostic_count,
            shadow_parse_diagnostic_count: unit_summary.shadow_parse_diagnostic_count,
            semantic_diagnostic_count: unit_summary.semantic_diagnostic_count,
            generated_semantic_diagnostic_count: unit_summary.generated_semantic_diagnostic_count,
            resolution_diagnostic_count: unit_summary.resolution_diagnostic_count,
            generated_resolution_diagnostic_count: unit_summary.generated_resolution_diagnostic_count,
            type_diagnostic_count: unit_summary.type_diagnostic_count,
            generated_type_diagnostic_count: unit_summary.generated_type_diagnostic_count,
          )
        end
        if options.verbose
          sources_by_path = aggregate.sources_by_path
        ...
        SemanticShadowSummary.new(
          ...
          generated_resolution_diagnostic_count: generated_resolution_diagnostics_by_unit.sum,
          type_diagnostic_count: type_diagnostics.size,
          generated_type_diagnostic_count: generated_type_diagnostics_by_unit.sum,
        )
      rescue ex
