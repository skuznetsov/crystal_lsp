...
module Crystal::MIR
  ...
  class LLVMIRGenerator
    ...
    @called_crystal_functions : Hash(String, {String, Int32}) = {} of String => {String, Int32}  # Track called Crystal functions (name => {return_type, arg_count}) for missing declaration generation
    ...
    private def emit_missing_crystal_function_stubs
      ...
      missing.each do |name, info|
        ...
        if stub = emit_dead_code_stub(name, return_type, arg_count)
          emit_raw stub
        else
          # Emit declare with matching arg count (all ptr) to avoid type mismatch
          param_list = (0...arg_count).map { |i| "ptr" }.join(", ")
    ...
    private def emit_dead_code_stub(name : String, return_type : String, arg_count : Int32 = 0) : String?
      ...
      # Build parameter list with proper types instead of varargs (...)
      # This prevents LLVM type mismatches at call sites.
      param_list = (0...arg_count).map { |i| "ptr %arg#{i}" }.join(", ")
    ...
    private def emit_builtin_override(func : Function) : Bool
      ...
      if mangled.includes?("$H$IDXQ$") && mangled.includes?("Hash$L")
        ...
        # Bail out if find_entry function isn't in the module — can't generate the
        # override wrapper without it. Fall through to normal lowering path.
        find_entry_func_exists = @module.functions.any? { |f| mangle_function_name(f.name) == find_entry_name } ||
                                 @emitted_functions.includes?(find_entry_name)
        return false unless find_entry_func_exists
          ...
          incoming = incoming_pairs.map do |(block, val)|
            ...
            if val_type_str && val_type_str == phi_type
              ...
              if existing && (match = existing.find { |e| e[2] == phi_type })
                ...
                arr << {convert_name, val_type_str, phi_type}
              end
    ...
    private def emit_call(inst : Call, name : String, func : Function)
      ...
      if callee_func && callee_func.id == func.id
        ...
        if alt = alternatives.find { |f| f.blocks.size > 2 || (f.blocks.size > 0 && f.blocks.any? { |b| b.instructions.size > 2 }) }
          ...
          @called_crystal_functions[alt_mangled] ||= {(alt_ret_type == "void" ? "ptr" : alt_ret_type), arg_strs.size}
      ...
      @called_crystal_functions[callee_name] ||= {(return_type == "void" ? "ptr" : return_type), inst.args.size}
    end
    ...
    private def emit_extern_call(inst : ExternCall, name : String)
      ...
      @called_crystal_functions[mangled_extern_name] ||= {(return_type == "void" ? "ptr" : return_type), inst.args.size}
    end
