...
module Crystal::MIR
  ...
  class LLVMIRGenerator
      ...
      if m = name.match(/\ANil\$_\$OR\$_(Int8|Int16|Int32|Int64|UInt8|UInt16|UInt32|UInt64)\$H\$(ADD|SUB)\$\$(Int8|Int16|Int32|Int64|UInt8|UInt16|UInt32|UInt64)\z/)
        ...
        if lhs == rhs
          ...
          return "; Nil | #{lhs}##{op}(#{rhs}) dead-code primitive\n" \
                 ...
                 "  %self.is_nil = icmp eq i32 %self.tid, #{nil_disc}\n" \
    ...
    # Post-process formatted call args to fix type annotation vs emitted type mismatches.
    # Each arg in the comma-separated string looks like "TYPE VALUE".
    ...
    private def fixup_call_arg_types(args : String) : String
      ...
      fixed = parts.map do |part|
        ...
        next part unless space_idx
        declared_type = part[0...space_idx]
        value_str = part[(space_idx + 1)..]
        # Only check SSA values (starting with %)
        next part unless value_str.starts_with?('%')
        # Skip zeroinitializer, null, literals
        next part if value_str == "zeroinitializer" || value_str == "null"
        # Look up actual emitted type
        emitted = @emitted_value_types[value_str]?
        next part unless emitted
        next part if emitted == declared_type
        # Type mismatch detected — insert conversion
        c = @cond_counter
        @cond_counter += 1
        if (declared_type == "float" || declared_type == "double") && emitted == "ptr"
          # ptr → float/double: decode packed scalar bits
          emit "%fixarg.#{c}.bits64 = ptrtoint ptr #{value_str} to i64"
          if declared_type == "double"
            emit "%fixarg.#{c}.val = bitcast i64 %fixarg.#{c}.bits64 to double"
          else
            emit "%fixarg.#{c}.bits32 = trunc i64 %fixarg.#{c}.bits64 to i32"
            emit "%fixarg.#{c}.val = bitcast i32 %fixarg.#{c}.bits32 to float"
          end
          ...
          if emitted == "double"
            emit "%fixarg.#{c}.bits = bitcast double #{value_str} to i64"
            emit "%fixarg.#{c}.val = inttoptr i64 %fixarg.#{c}.bits to ptr"
          else
            emit "%fixarg.#{c}.bits = bitcast float #{value_str} to i32"
            emit "%fixarg.#{c}.ext = zext i32 %fixarg.#{c}.bits to i64"
            emit "%fixarg.#{c}.val = inttoptr i64 %fixarg.#{c}.ext to ptr"
          end
          "ptr %fixarg.#{c}.val"
        elsif declared_type == "ptr" && emitted.starts_with?('i') && !emitted.includes?(".union")
          # int → ptr
          emit "%fixarg.#{c}.val = inttoptr #{emitted} #{value_str} to ptr"
          "ptr %fixarg.#{c}.val"
        elsif declared_type.starts_with?('i') && !declared_type.includes?(".union") && emitted == "ptr"
          # ptr → int
          emit "%fixarg.#{c}.val = ptrtoint ptr #{value_str} to #{declared_type}"
          "#{declared_type} %fixarg.#{c}.val"
        elsif (declared_type == "float" || declared_type == "double") && emitted.starts_with?('i') && !emitted.includes?(".union")
          # int → float
          emit "%fixarg.#{c}.val = sitofp #{emitted} #{value_str} to #{declared_type}"
          "#{declared_type} %fixarg.#{c}.val"
        elsif declared_type.starts_with?('i') && !declared_type.includes?(".union") && (emitted == "float" || emitted == "double")
          # float → int
          emit "%fixarg.#{c}.val = fptosi #{emitted} #{value_str} to #{declared_type}"
          "#{declared_type} %fixarg.#{c}.val"
        else
          # Unknown mismatch — leave as-is (will likely fail at opt but at least we tried)
          part
        end
      end
      fixed.join(", ")
    ...
    private def emit_runtime_declarations
      ...
      emit_raw "@.uint_fmt = private constant [4 x i8] c\"%u\\0A\\00\"\n"
      emit_raw "@.uint_fmt_no_nl = private constant [3 x i8] c\"%u\\00\"\n"
      emit_raw "@.long_fmt = private constant [5 x i8] c\"%ld\\0A\\00\"\n"
      emit_raw "@.long_fmt_no_nl = private constant [4 x i8] c\"%ld\\00\"\n"
      emit_raw "@.ulong_fmt = private constant [5 x i8] c\"%lu\\0A\\00\"\n"
      emit_raw "@.ulong_fmt_no_nl = private constant [4 x i8] c\"%lu\\00\"\n"
      ...
      emit_raw "  call i64 @write(i32 1, ptr %data, i64 %bs64)\n"
      emit_raw "  br label %done\n"
      emit_raw "done:\n"
      emit_raw "  ret void\n"
      emit_raw "}\n\n"
      ...
      emit_raw "print_true:\n"
      emit_raw "  call i64 @write(i32 1, ptr @.str.true, i64 4)\n"
      emit_raw "  ret void\n"
      emit_raw "print_false:\n"
      emit_raw "  call i64 @write(i32 1, ptr @.str.false, i64 5)\n"
      emit_raw "  ret void\n"
      emit_raw "}\n\n"
    ...
    private def emit_builtin_override(func : Function) : Bool
      ...
      when "String$Hbyte_slice$Q$$Int32_Int32"
        ...
        emit_raw "  store i32 #{string_disc}, ptr %ret.tid\n"
      ...
      when "String$Hindex$$Char"
        ...
        emit_raw "  store i32 #{int32_disc}, ptr %f_tid\n"
      ...
      when "Slice$LUInt8$R$Hrindex$$UInt8"
        ...
        emit_raw "  store i32 #{int32_disc}, ptr %f_tag\n"
        emit_raw "  %f_pay = getelementptr {i32, [8 x i8]}, ptr %f_u, i32 0, i32 1\n"
        emit_raw "  store i32 %i, ptr %f_pay, align 4\n"
        emit_raw "  %f_val = load #{union_type}, ptr %f_u\n"
        emit_raw "  ret #{union_type} %f_val\n"
      ...
      if mangled.includes?("$H$IDXQ$") && mangled.includes?("Hash$L")
        ...
        emit_raw "  %found = icmp ne i32 %tid, #{nil_variant_id}\n"
      ...
      if mangled == "Number$H$CMP$$Number"
        ...
        emit_raw "  store i32 #{int32_disc}, ptr %pos_tid\n"
        ...
        emit_raw "  store i32 #{int32_disc}, ptr %neg_tid\n"
        ...
        emit_raw "  store i32 #{int32_disc}, ptr %zero_tid\n"
    ...
    private def emit_function(func : Function)
      ...
      param_types = func.params.map do |p|
        ...
        record_emitted_type("%#{llvm_name}", llvm_type)
      ...
      if mangled_name.ends_with?("$Hobject_id") && return_type == "i64" && param_types.size == 1
        ...
        emit_raw "define #{return_type} @#{mangled_name}(#{param_types.join(", ")}) {\n"
        emit_raw "entry:\n"
        if self_llvm_type == "ptr"
          emit_raw "  %addr = ptrtoint ptr %self to i64\n"
        elsif self_llvm_type == "float"
          emit_raw "  %bits = bitcast float %self to i32\n"
          emit_raw "  %addr = zext i32 %bits to i64\n"
        elsif self_llvm_type == "double"
          emit_raw "  %addr = bitcast double %self to i64\n"
        else
          emit_raw "  %addr = zext #{self_llvm_type} %self to i64\n"
        end
        emit_raw "  ret i64 %addr\n"
        emit_raw "}\n\n"
        return
      ...
      func.blocks.each do |block|
        block.instructions.each do |inst|
          ...
          phi.incoming.each do |(pred_block_id, val_id)|
            ...
            # Skip if already union-typed. Check multiple sources since ExternCall
            # return type overrides can collapse union types to scalars in @value_types
            # while the actual emitted value is a union (via ext_s2u wrapping).
            next if val_llvm_type && val_llvm_type.includes?(".union")
            slot_llvm_type = @cross_block_slot_types[val_id]?
            next if slot_llvm_type && slot_llvm_type.includes?(".union")
            # Also check the defining instruction's MIR type — if it declares a union type,
            # the emission code will wrap the scalar result into a union (ext_s2u),
            # so the phi predecessor should NOT add another union wrap.
            if def_inst = find_def_inst(val_id)
              def_llvm = @type_mapper.llvm_type(def_inst.type)
              next if def_llvm.includes?(".union")
            end
    ...
    private def prepass_collect_phi_union_to_ptr_extracts(func : Function)
      ...
      func.blocks.each do |block|
        block.instructions.each do |inst|
          ...
          # Only pre-register union->ptr extracts when the phi is explicitly ptr-emitted
          # (either MIR ptr phi, or prepass-resolved ptr for a union-typed MIR phi).
          # Heuristic ptr fallback for union phis causes invalid downstream type mixing.
          emit_ptr_phi = (prepass_phi_type == TypeRef::POINTER && phi_mir_type.includes?(".union")) ||
                         phi_mir_type == "ptr"
    ...
    private def emit_gep_dynamic(inst : GetElementPtrDynamic, name : String)
      ...
      if index_type_str.includes?(".union")
        ...
        idx_nil_vid = nil_variant_id_for_union_type(index_type_str) || 0
        emit "%#{base_name}.idx_is_nil = icmp eq i32 %#{base_name}.idx_type_id, #{idx_nil_vid}"
      ...
      if op.starts_with?("icmp")
        ...
        if wrap_union_result
          ...
          if descriptor = @module.get_union_descriptor(inst.type)
            ...
            if best_variant
              variant_type_id = best_variant.type_ref.id.to_i32  # Use global type_ref.id
      ...
      when .neg?
        if operand_llvm_type.includes?(".union")
          ...
          if descriptor = @module.get_union_descriptor(inst.type)
            ...
            if float_variant
              variant_type_id = float_variant.type_ref.id.to_i32  # Use global type_ref.id
              ...
              if int_variant
                variant_type_id = int_variant.type_ref.id.to_i32  # Use global type_ref.id
    ...
    private def emit_cast(inst : Cast, name : String)
      ...
      # Guard: MIR may request ptr_to_int while the emitted source is already non-pointer.
      # Normalize to a valid cast in the actual source domain.
    ...
    private def emit_phi(inst : Phi, name : String)
      ...
      if is_union_type
        ...
        has_non_union_incoming = incoming_pairs.any? do |(block, val)|
          val_type = @value_types[val]?
          val_llvm_type = val_type ? @type_mapper.llvm_type(val_type) : nil
          # Any typed non-union incoming must be normalized to the phi union type.
          val_llvm_type && !val_llvm_type.includes?(".union")
        end
    ...
    private def emit_call(inst : Call, name : String, func : Function)
      ...
      args = if use_callee_params && callee_func
               call_args.map_with_index { |a, i|
                 ...
                 # If types match, use directly (but handle union 0/null and float literals specially)
                   ...
                   elsif expected_llvm_type == "ptr" && emitted_actual.includes?(".union")
                     # value_ref loaded a union from a cross-block slot but we need ptr.
                     # Extract payload pointer from the union struct.
                     c = @cond_counter
                     @cond_counter += 1
                     emit "%eq_union_to_ptr.#{c}.tmp = alloca #{emitted_actual}, align 8"
                     emit "store #{emitted_actual} #{normalize_union_value(val, emitted_actual)}, ptr %eq_union_to_ptr.#{c}.tmp"
                     emit "%eq_union_to_ptr.#{c}.pp = getelementptr #{emitted_actual}, ptr %eq_union_to_ptr.#{c}.tmp, i32 0, i32 1"
                     emit "%eq_union_to_ptr.#{c}.val = load ptr, ptr %eq_union_to_ptr.#{c}.pp, align 4"
                     "ptr %eq_union_to_ptr.#{c}.val"
                  ...
                  val_ref_emitted = @emitted_value_types[val_ref]?
                  ...
                  val = value_ref(a)
                  if val == "0" || val == "null"
                    # Null pointer value in integer context.
                    "#{expected_llvm_type} 0"
                  else
                  ...
                  if emitted_type && emitted_type == expected_llvm_type
                   ...
                   scalar_variant_type_id = if desc = @module.union_descriptors[param_type]?
                     ...
                     if matching_variant
                       ...
                       non_nil ? non_nil.type_ref.id.to_i32 : 0
                     end
                   else
                     0
                   end
               ...
               call_args.map { |a|
                 ...
                 if arg_llvm_type == "void"
                   ...
                   val = value_ref(a)
                   # Check emitted type — MIR type can disagree with actual emitted LLVM type
                   emitted = @emitted_value_types[val]?
                   if emitted && emitted != arg_llvm_type
                     if (arg_llvm_type == "float" || arg_llvm_type == "double") && emitted == "ptr"
                       # Packed scalar in ptr: decode bit-pattern
                       c = @cond_counter
                       @cond_counter += 1
                       emit "%fb_ptr_to_fp.#{c}.bits64 = ptrtoint ptr #{val} to i64"
                       if arg_llvm_type == "double"
                         emit "%fb_ptr_to_fp.#{c}.val = bitcast i64 %fb_ptr_to_fp.#{c}.bits64 to double"
                       else
                         emit "%fb_ptr_to_fp.#{c}.bits32 = trunc i64 %fb_ptr_to_fp.#{c}.bits64 to i32"
                         emit "%fb_ptr_to_fp.#{c}.val = bitcast i32 %fb_ptr_to_fp.#{c}.bits32 to float"
                       end
                       next "#{arg_llvm_type} %fb_ptr_to_fp.#{c}.val"
                     elsif arg_llvm_type == "ptr" && emitted.includes?(".union")
                       # Union value where ptr expected: extract payload
                       c = @cond_counter
                       @cond_counter += 1
                       emit "%fb_union_to_ptr.#{c}.tmp = alloca #{emitted}, align 8"
                       emit "store #{emitted} #{normalize_union_value(val, emitted)}, ptr %fb_union_to_ptr.#{c}.tmp"
                       emit "%fb_union_to_ptr.#{c}.pp = getelementptr #{emitted}, ptr %fb_union_to_ptr.#{c}.tmp, i32 0, i32 1"
                       emit "%fb_union_to_ptr.#{c}.val = load ptr, ptr %fb_union_to_ptr.#{c}.pp, align 4"
                       next "ptr %fb_union_to_ptr.#{c}.val"
                     elsif emitted == "ptr" && arg_llvm_type.starts_with?('i') && !arg_llvm_type.includes?(".union")
                       # Ptr where int expected: ptrtoint
                       c = @cond_counter
                       @cond_counter += 1
                       emit "%fb_ptrtoint.#{c} = ptrtoint ptr #{val} to #{arg_llvm_type}"
                       next "#{arg_llvm_type} %fb_ptrtoint.#{c}"
                     end
                   end
                   # For union types, use zeroinitializer instead of 0
      ...
      # Safety net: verify each formatted arg's type annotation matches the value's
      ...
      # Mark call results as packed scalars when the callee is an instance method
      # on an abstract value type (Int, Number, etc.) but the actual self argument
      # is a concrete scalar. The callee wraps its return via inttoptr, so the
      # returned ptr contains a packed scalar, not a real pointer address.
      # IMPORTANT: Only when callee's return type is NOT a reference/class type —
      # methods like Int#to_s return String (a real pointer), not a packed scalar.
      if return_type == "ptr" && callee_func && callee_func.name.includes?('#') && inst.args.size >= 1
        self_arg_type = @value_types[inst.args[0]]?
        if self_arg_type
          self_arg_llvm = @type_mapper.llvm_type(self_arg_type)
          if (self_arg_llvm.starts_with?('i') && !self_arg_llvm.includes?(".union")) ||
             self_arg_llvm == "float" || self_arg_llvm == "double" ||
             @inttoptr_value_ids.includes?(inst.args[0])
            # Verify the return type is a value type, not a reference/class type.
            ret_info = @module.type_registry.get(callee_func.return_type)
            is_ref_return = ret_info && (ret_info.kind == TypeKind::Reference ||
                                         ret_info.kind == TypeKind::Array ||
                                         ret_info.kind == TypeKind::Pointer)
            @inttoptr_value_ids.add(inst.id) unless is_ref_return
          end
        end
      end
    ...
    private def emit_extern_call(inst : ExternCall, name : String)
      ...
      if mangled_extern_name == "nil_" && inst.args.size == 1
        ...
        if arg_llvm_type.includes?(".union")
          ...
          nil_disc = nil_variant_id_for_union_type(arg_llvm_type) || 0
          emit "#{name} = icmp eq i32 %nil_check_tag.#{c}, #{nil_disc}"  # Nil discriminator (0 in hybrid scheme)
    ...
    private def emit_global_store(inst : GlobalStore, name : String)
      ...
      # Final type safety net: value_ref may synthesize a casted SSA value whose
    ...
    private def emit_union_wrap(inst : UnionWrap, name : String)
      ...
      # Fix: when wrapping a ptr into a union with multiple class/reference variants,
      # the static variant_type_id may be wrong (e.g., always 0) because union-typed
      # parameters are passed as raw ptrs, losing the type_id. Read the class type_id
      # from the object header at runtime and map to the correct union variant_type_id.
      runtime_tid_emitted = false
      if val_type_str == "ptr" && val != "null"
        if union_desc = @module.get_union_descriptor(inst.union_type)
          ...
          if ref_variants.size > 1
            # Multiple class variants — need runtime class type_id dispatch.
            # Use a stack dummy so we can safely load even if ptr is null.
            emit "%#{base_name}.rt_dummy = alloca i32, align 4"
            emit "store i32 0, ptr %#{base_name}.rt_dummy"
            emit "%#{base_name}.rt_is_null = icmp eq ptr #{val}, null"
            emit "%#{base_name}.rt_safe = select i1 %#{base_name}.rt_is_null, ptr %#{base_name}.rt_dummy, ptr #{val}"
            emit "%#{base_name}.rt_class_tid = load i32, ptr %#{base_name}.rt_safe"
            # Build a select chain: for each ref variant, check if class_tid matches.
            # Store type_ref.id (globally unique) as discriminator, not sequential type_id.
            fallback_gid = variant_global_id(inst.union_type, inst.variant_type_id)
            result = "#{fallback_gid}"  # fallback
            ref_variants.each do |variant|
              emit "%#{base_name}.rt_m#{variant.type_id} = icmp eq i32 %#{base_name}.rt_class_tid, #{variant.type_ref.id}"
              emit "%#{base_name}.rt_s#{variant.type_id} = select i1 %#{base_name}.rt_m#{variant.type_id}, i32 #{variant.type_ref.id}, i32 #{result}"
              result = "%#{base_name}.rt_s#{variant.type_id}"
            end
            # If union has a nil variant, select nil_vid when ptr is null
            if nil_vid != nil
              emit "%#{base_name}.rt_final = select i1 %#{base_name}.rt_is_null, i32 #{nil_vid}, i32 #{result}"
              result = "%#{base_name}.rt_final"
            end
            emit "store i32 #{result}, ptr %#{base_name}.type_id_ptr"
            runtime_tid_emitted = true
          end
        end
      end

      unless runtime_tid_emitted
        # Use globally unique type_ref.id as discriminator
        global_vid = variant_global_id(inst.union_type, inst.variant_type_id)
        if val_type_str == "ptr" && nil_vid != nil && global_vid != nil_vid
          emit "%#{base_name}.is_null = icmp eq ptr #{val}, null"
          emit "%#{base_name}.type_id = select i1 %#{base_name}.is_null, i32 #{nil_vid}, i32 #{global_vid}"
          emit "store i32 %#{base_name}.type_id, ptr %#{base_name}.type_id_ptr"
        else
          emit "store i32 #{global_vid}, ptr %#{base_name}.type_id_ptr"
        end
      end
    ...
    private def emit_union_unwrap(inst : UnionUnwrap, name : String)
      ...
      if union_type_is_union
        ...
        emitted_union_val_type = @emitted_value_types[union_val]?
        store_val = if emitted_union_val_type && emitted_union_val_type == union_type
                      normalize_union_value(union_val, union_type)
                    else
                      coerce_union_value_for_type("#{base_name}.unwrap", union_val, union_type)
                    end
        ...
        if inst.safe
          ...
          # Compare against globally unique type_ref.id, not sequential variant index
          unwrap_global_vid = inst.type.id.to_i32
          emit "%#{base_name}.type_match = icmp eq i32 %#{base_name}.actual_type_id, #{unwrap_global_vid}"
        ...
        elsif (result_type == "float" || result_type == "double") && actual_union_val_type == "ptr"
          # Union passed as ptr pointing to {i32 type_id, payload} in memory.
          # Load the value type from the payload area at byte offset 4.
          emit "%#{base_name}.pay_ptr = getelementptr i8, ptr #{union_val}, i32 4"
          emit "#{name} = load #{result_type}, ptr %#{base_name}.pay_ptr, align 4"
        else
          # Fallback: bitcast — record as ptr since that's what bitcast ptr→ptr produces
          emit "#{name} = bitcast ptr #{union_val} to ptr"
          record_emitted_type(name, "ptr")
          @value_types[inst.id] = inst.type
          return
    ...
    private def emit_union_type_id_get(inst : UnionTypeIdGet, name : String)
      ...
      if union_type_is_union
        ...
        store_val = if emitted_union_val_type && emitted_union_val_type == union_type
                      ...
                      coerce_union_value_for_type("#{base_name}.typeid", union_val, union_type)
                    end
        emit "store #{union_type} #{store_val}, ptr %#{base_name}.union_ptr"
    ...
    private def emit_union_is(inst : UnionIs, name : String)
      ...
      if union_type_is_union
        ...
        store_val = if emitted_union_val_type && emitted_union_val_type == union_type
                      ...
                      coerce_union_value_for_type("#{base_name}.is", union_val, union_type)
                    end
        emit "store #{union_type} #{store_val}, ptr %#{base_name}.union_ptr"
        ...
        emit "%#{base_name}.type_id_ptr = getelementptr #{union_type}, ptr %#{base_name}.union_ptr, i32 0, i32 0"
        emit "%#{base_name}.actual_type_id = load i32, ptr %#{base_name}.type_id_ptr"
        # Compare against globally unique type_ref.id
        is_global_vid = variant_global_id(union_type, inst.variant_type_id)
        emit "#{name} = icmp eq i32 %#{base_name}.actual_type_id, #{is_global_vid}"
      else
          ...
          if union_desc
            union_desc.variants.each do |v|
              if v.type_ref == val_type_ref
                val_type_id = v.type_ref.id.to_i32  # Use global type_ref.id
                break
              end
            end
          end
    ...
    private def emit_string_interpolation(inst : StringInterpolation, name : String)
      ...
      inst.parts.each_with_index do |part_id, idx|
        ...
        # Use HIR-level part_types as authoritative source when @value_types is
        # POINTER/nil.  Call results (e.g., hash lookups) often get @value_types
        # set to POINTER even when the actual value is Int32 or another type.
        if hir_pt = inst.part_types.try(&.[idx]?)
          if hir_pt == TypeRef::CHAR && (part_type == TypeRef::INT32 || part_type.nil?)
            part_type = TypeRef::CHAR
          elsif part_type == TypeRef::POINTER || part_type.nil?
            # Trust HIR part_type over default POINTER — it tracks the actual
            # Crystal type of the interpolated expression
            part_type = hir_pt unless hir_pt == TypeRef::POINTER
          end
        end
        ...
        if part_type == TypeRef::STRING || part_type == TypeRef::POINTER || part_type.nil?
          ...
          if part_llvm_type == "i32"
            ...
            # Determine payload types from union variant tokens.
            # Check ALL variants (not just the last one) to find int/bool variants.
            union_tokens = union_variant_tokens_for_llvm_union(part_llvm_type)
            has_int32 = union_tokens.any? { |t| t == "Int32" || t == "UInt32" }
            has_int64 = union_tokens.any? { |t| t == "Int64" || t == "UInt64" }
            has_bool = union_tokens.any? { |t| t == "Bool" }
            has_ptr = union_tokens.any? { |t| t == "String" || t == "Pointer" || (!t.in?("Nil", "Bool", "Int8", "UInt8", "Int16", "UInt16", "Int32", "UInt32", "Int64", "UInt64", "Int128", "UInt128", "Float32", "Float64", "Char") && t != "Nil") }

            interp_nil_vid = nil_variant_id_for_union_type(part_llvm_type) || 0
            emit "%#{base_name}.is_nil#{idx} = icmp eq i32 %#{base_name}.tid#{idx}, #{interp_nil_vid}"

            if has_int32 && has_ptr
              # Mixed union (e.g., Int32 | String): dispatch based on discriminator.
              # Find the Int32 variant discriminator to select the right conversion.
              int_vid = find_variant_id_by_type_name(part_llvm_type, part_type, "Int32") ||
                        find_variant_id_by_type_name(part_llvm_type, part_type, "UInt32")
              if int_vid
                ...
                emit "%#{base_name}.conv#{idx} = select i1 %#{base_name}.is_nil#{idx}, ptr @.str.empty, ptr %#{base_name}.non_nil#{idx}"
              else
                # Fallback: load as ptr
                emit "%#{base_name}.str_ptr#{idx} = load ptr, ptr %#{base_name}.payload_ptr#{idx}, align 4"
      ...
      when Return
        ...
        if @current_return_type == "void"
          ...
          if @current_return_type.includes?(".union") && !val_llvm_type.includes?(".union")
            ...
            ret_nonnil_vid = first_non_nil_variant_id_for_union_type(@current_return_type) || TypeRef::INT32.id.to_i32
    ...
    private def variant_global_id(union_type_ref : TypeRef, variant_type_id : Int32) : Int32
      if union_desc = @module.get_union_descriptor(union_type_ref)
        if variant = union_desc.variants.find { |v| v.type_id == variant_type_id }
          if variant.type_ref == TypeRef::NIL || variant.type_ref == TypeRef::VOID
            return 0
          end
