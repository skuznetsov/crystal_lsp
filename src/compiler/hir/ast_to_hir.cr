...
module Crystal::HIR
  ...
  class AstToHir
    ...
    private def register_concrete_class(node : CrystalV2::Compiler::Frontend::ClassNode, class_name : String, is_struct : Bool)
      ...
      if ENV.has_key?("DEBUG_CLASS_INFO") && class_name == "Crystal::MachO"
        ivar_dump = ivars.map { |iv| "#{iv.name}:#{get_type_name_from_ref(iv.type)}" }.join(", ")
    ...
    private def lower_return(ctx : LoweringContext, node : CrystalV2::Compiler::Frontend::ReturnNode) : ValueId
      ...
      if @inline_yield_block_body_depth == 0 && @inline_yield_proc_depth == 0
        if inline_return = @inline_yield_return_stack.last?
